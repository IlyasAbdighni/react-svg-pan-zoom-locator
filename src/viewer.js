...
export default class ReactSVGPanZoom extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (
      value.viewerWidth !== nextProps.width ||
      value.viewerHeight !== nextProps.height
    ) {
    ...
    if (
      value.scaleFactorMin !== nextProps.scaleFactorMin ||
      value.scaleFactorMax !== nextProps.scaleFactorMax
    ) {
      nextValue = setZoomLevels(
        nextValue,
        nextProps.scaleFactorMin,
        nextProps.scaleFactorMax
      );
      needUpdate = true;
    }
  ...
  getSvgStyle(cursor) {
    ...
    if (
      this.props.detectPinchGesture ||
      [TOOL_PAN, TOOL_AUTO].indexOf(this.getTool()) !== -1
    ) {
  ...
  setPointOnViewerCenter(SVGPointX, SVGPointY, zoomLevel) {
    let nextValue = setPointOnViewerCenter(
      this.getValue(),
      SVGPointX,
      SVGPointY,
      zoomLevel
  ...
  changeTool(tool) {
    this.setState({ tool });
    this.props.onChangeTool && this.props.onChangeTool(tool);
  }
  ...
  render() {
    ...
    if (tool === TOOL_PAN)
      cursor = cursorPolyfill(
        value.mode === MODE_PANNING ? "grabbing" : "grab"
      );
    ...
    if (tool === TOOL_POINT) cursor = cursorPolyfill("point");

    if (panningWithToolAuto) cursor = cursorPolyfill("grabbing");

    let blockChildEvents =
      [TOOL_PAN, TOOL_ZOOM_IN, TOOL_ZOOM_OUT].indexOf(tool) >= 0;
    ...
    return (
      ...
      >
        ...
        <Node
          scale={this.state.value.a}
          left={this.state.value.e}
          top={this.state.value.f}
        />
...
ReactSVGPanZoom.propTypes = {
  ...
  //miniature position
  miniaturePosition: PropTypes.oneOf([
    POSITION_NONE,
    POSITION_RIGHT,
    POSITION_LEFT
  ]),
  ...
  children: function(props, propName, componentName) {
    ...
    if (
      !prop.props.hasOwnProperty("width") ||
      !prop.props.hasOwnProperty("height")
    ) {
